...
class ImagingPath(MatrixGroup):
    ...
    def display(self, onlyPrincipalAndAxialRays=True,
        ...
        self.figure.designParams['removeBlockedRaysCompletely'] = removeBlockedRaysCompletely
        # todo: since they are all defaults, we could remove some of these arguments from the display method

        self.figure.display(comments=comments, title=self.label, backend='matplotlib', display3D=False)
    ...
    def saveFigure(self, filepath,
        ...
        self.figure.display(comments=comments, title=self.label, backend='matplotlib', display3D=False, filepath=filepath)
