...
def optimalPinholeSize():
    ...
    pinholeIdeal = illumination.imageSize()
    illumination.append(Aperture(diameter=pinholeIdeal))

    return pinholeIdeal

def illuminationPath(pinholeFactor=None, focalspot=None):

    illumination = ImagingPath()
    illumination.append(Space(d=focalspot))
    illumination.append(Lens(f=objFocalLength))
    illumination.append(Space(d=105))
    illumination.append(Lens(f=100))
    illumination.append(Space(d=200))
...
for pinhole in pinholeModifier:
    ...
    for z in positions:
        ...
        newPosition = 5 + (z*0.000001)  # Working distance of the objective + position
        #print(newPositions)
